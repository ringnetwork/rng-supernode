...
function initRPC() {
  ...
  var db = require('rng-core/db/db.js');
	...
	server.expose('createMinerAddress', function(args, opt, cb){
		mutex.lock(['rpc_createMinerAddress'], function(unlock){
			db.query('select * from supernode where address=?', [my_address], function(rows){
				if(rows.length>=1){
					unlock()
	...
	server.expose('claimDeposit', function(args, opt, cb){
		...
		mutex.lock(['claimDeposit'], function(unlock) {
			create_deposit_address.getDepositAddressBySupernodeAddress(null, my_address, function(err, deposit_address){
				...
				wallet.sendMultiPayment({
					...
					signWithLocalPrivateKey: supernode.signWithLocalPrivateKey
				}, function(err, result) {
					if(err) {
						unlock()
						return cb(err)
